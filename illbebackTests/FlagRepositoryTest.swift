...
//    }
    ...
    private func offer() -> Flag {
        ...
        return offer(offeredFlag)
    }
    
    private func offer(offeredFlag: Flag) -> Flag {
        offeredFlag.invite("invitee").inviteSuccess()
    ...
    private func receive(offeredFlag: Flag) -> Flag {
        ...
        inviteeRepository.receive("originator", to: "invitee", flag: receivedFlag, onNew: { flag in try! flag.receiveNewSuccess(); self.inviteeRepository.add(flag) }, onUpdate: {flag in try! flag.receiveUpdateSuccess() }, onAck: { _ in XCTFail() })
        return inviteeFlag()
    }
    
    private func receiveByOriginator(offeredFlag: Flag) -> Flag {
        let receivedFlag = transfer(offeredFlag)
        originatorRepository.receive("invitee", to: "originator", flag: receivedFlag, onNew: { flag in try! flag.receiveNewSuccess(); self.inviteeRepository.add(flag) }, onUpdate: {flag in try! flag.receiveUpdateSuccess() }, onAck: { _ in XCTFail() })
        return originatorFlag()
    }
    ...
    private func decline(receivedFlag: Flag) -> Flag {
        let invitee = receivedFlag.declining("invitee")
        let returnedFlag = transfer(receivedFlag)
        var calledBack = false
        originatorRepository.receive("invitee", to: "originator", flag: returnedFlag, onNew: { _ in XCTFail() }, onUpdate: { _ in XCTFail() }, onAck: { _ in calledBack = true })
